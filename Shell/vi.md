...
以 : 开始的命令你需要输入 &lt;enter&gt;回车，例如 — 如果我写成 
...
3. **拷贝/粘贴** 
    - `P` → 粘贴（陈皓注：p/P都可以，p是表示在当前位置之后，P表示在当前位置之前）
    - `yy` → 拷贝当前行当行于 `ddP`
...
4. 按单词移动：  
    1.w → 到下一个单词的开头。
    2.e → 到下一个单词的结尾。&gt; 如果你认为单词是由默认方式，那么就用小写的e和w。默认上来说，一个单词由字母，数字和下划线组成（陈皓注：程序变量） &gt; 如果你认为单词是由blank字符分隔符，那么你需要使用大写的E和W。（陈皓注：程序语句）  
...
&lt;action&gt;a&lt;object&gt; 和 &lt;action&gt;i&lt;object&gt;
 action可以是任何的命令，如 d (删除), y (拷贝), v (可以视模式选择)。 object 可能是： 
    - w 一个单词， 
    - W 一个以空格为分隔的单词， 
    - s 一个句字， 
    - p 一个段落。也可以是一个特别的字符："、 '、 )、 }、]。
...
&lt;C-n&gt; 和 &lt;C-p&gt;在 Insert 模式下，你可以输入一个词的开头，然后按 
...
> 在一个只有一行且这一行只有“1”的文本中，键入如下命令： qa Yp &lt;C-a&gt; q
...
前面，我们看到了 &lt;C-v&gt;的示例 （在Windows下应该是&lt;C-q&gt;），我们可以使用v 和 V。一但被选好了，你可以做下面的事：
